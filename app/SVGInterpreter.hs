...
drawingToSVG drawing@((_, shape, _):_)
    ...
    | isNothing (shapeToSVG shape) = BlazeSvg.docType ! SvgAttr.version "1.1" ! SvgAttr.width "1000" ! SvgAttr.height "1000"
    -- otherwise parse the drawings into SVGs
    | otherwise = BlazeSvg.docTypeSvg ! SvgAttr.version "1.1" ! SvgAttr.width "1000" ! SvgAttr.height "1000" $ do mapSVGs (map drawToSVG drawing)
...
-- Function to take a scale transformation and turn it into an SVG scale
parseScale :: Transform -> Maybe BlazeSvg.Attribute
parseScale (Scale (Vector x y)) = Just (SvgAttr.transform (BlazeSvg.scale x y))

-- Function to take a rotate transformation and turn it into an SVG rotation
